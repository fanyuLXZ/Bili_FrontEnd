...
<template>
  <div class="el-picker-panel el-date-picker">
    <div class="el-picker-panel__body-wrapper"><!---->
      <div class="el-picker-panel__body"><!---->
        <div class="el-date-picker__header">
          <button type="button"
                  class="el-picker-panel__icon-btn el-date-picker__prev-btn el-icon-d-arrow-left" @click.stop="left_best_click"></button>
          <button type="button"
                  class="el-picker-panel__icon-btn el-date-picker__prev-btn el-icon-arrow-left"
                  @click.stop="left_click"
                  v-if="model==='day'"></button>
          <span class="el-date-picker__header-label"
                v-if="model==='day'||model==='month'"
                @click.stop="model='year'">{{ year }} 年</span>
          <span class="el-date-picker__header-label"
                v-if="model==='day'"
                @click.stop="model='month'">{{ month }} 月</span>
          <span class="el-date-picker__header-label"
                v-if="model==='year'">{{ getFirstYear + ' 年' +' - '+ getLastYear +' 年' }}</span>
          <button type="button"
                  class="el-picker-panel__icon-btn el-date-picker__next-btn el-icon-d-arrow-right" @click.stop="right_best_click"></button>
          <button type="button"
                  class="el-picker-panel__icon-btn el-date-picker__next-btn el-icon-arrow-right"
                  v-if="model==='day'" @click.stop="right_click"></button>
        </div>
        <div class="el-picker-panel__content">
          <table cellspacing="0" cellpadding="0" v-show="model==='day'" class="el-date-table">
            <tr><!---->
              <th>日</th>
              <th>一</th>
              <th>二</th>
            ...
            <tr class="el-date-table__row" v-for="(day_table_data_row,index) in day_table_data" :key="index">
              <td @click.stop="day_table_data_cel.isDisabled?'':day_table_data_cel.class==='available'?
              setDay(day_table_data_cel.num):
                day_table_data_cel.class==='prev-month'?
                setPrevMonthDay(day_table_data_cel.num):
                setNextMonthDay(day_table_data_cel.num)"
              :class="
              (day_table_data_cel.num===current_date_object.getDate()
              &&month===current_date_object.getMonth()+1
              &&year===current_date_object.getFullYear()
              &&day_table_data_cel.class==='available'?'current ':' ')+day_table_data_cel.class"
                  :key="index"
                  v-for="(day_table_data_cel,index) in day_table_data_row">
                <a>{{day_table_data_cel.num}}</a>
              </td>
            </tr>
          </table>
          <table class="el-year-table" v-show="model==='year'">
            <tr v-for="(year_table_data_row,index) in year_table_data" :key="index">
              <td @click="year_table_data_cel.isDisabled?'':setYear(year_table_data_cel.num)" :class="year_table_data_cel.class" :key="index" v-for="(year_table_data_cel,index) in year_table_data_row">
                <a class="cell">{{ year_table_data_cel.num }}</a>
              </td>
            </tr>
          </table>
          <table class="el-month-table" v-show="model==='month'">
            <tr v-for="(month_table_data_row,index) in month_table_data" :key="index">
              <td @click="month_table_data_cel.isDisabled?'':setMonth(month_table_data_cel.num)" :class="month_table_data_cel.class" :key="index" v-for="(month_table_data_cel,index) in month_table_data_row">
                <a class="cell">{{convertToChinaNum(month_table_data_cel.num)}}月</a>
              </td>
            </tr>
          </table>
        </div>
    ...
    <div class="el-picker-panel__footer" style="display: none;">
      <a href="JavaScript:" class="el-picker-panel__link-btn">此刻</a>
      <button type="button" class="el-picker-panel__btn">确定</button>
    </div>
  </div>
</template>

<script>
import  {convertToChinaNum} from "g-public/js/utils"
export default {
  ...
  data(){
    return{
      ...
      month: 10,
      day: 10,
      current_date: this.date_format(new Date(this.year,this.month-1,this.day)),
      current_date_object: new Date(this.year,this.month-1,this.day)
    }
  },
  props:['date'],
  computed:{
    getFirstYear(){
      return this.year?this.year.toString().substring(0,3)+"0":"";
    },
    getLastYear(){
      return this.year?this.year.toString().substring(0,3)+"9":"";
    }
  },
  mounted() {
    if (!this.date) {
      this.current_date = this.date_format(this.now)
    }else {
  ...
  methods:{
    ...
    getYearData(){
      let data = []
      let data_row = []
      let fist_year = Number(this.getFirstYear)
      let last_year = Number(this.getLastYear)
      for (let i = fist_year;i <= last_year;i++){
        let item = {num:i,class:'available',isDisabled:false}
        if(i>this.current_date_object.getFullYear()){
          item.isDisabled=true
          item.class+=" disabled"
        }else if (i===this.current_date_object.getFullYear()){
          item.class+=" current"
        }
        if (data_row.length<4){
          data_row.push(item)
        }else {
          data.push(data_row)
          data_row = []
    ...
    getMonthData(){
      ...
      // 判断是否是当年
      let is_this_year = this.year===this.now.getFullYear()
      // 判断是否是以后
      let is_later = this.now.getFullYear()<this.year
      if (is_later){
        data.forEach((v,i)=>{
            v.forEach((val,j)=>{
              data[i][j]={num:val,isDisabled:true,class:'available disabled'}
            })
        })
      }else if (is_this_year){
        data.forEach((v,i)=>{
          v.forEach((val,j)=>{
            if (val>this.now.getMonth()+1){
              data[i][j]={num:val,isDisabled:true,class:'available disabled'}
            }else if(val===this.now.getMonth()+1){
              data[i][j]={num:val,isDisabled:false,class:'available current'}
            }else {
              data[i][j]={num:val,isDisabled:false,class:'available '}
            }
          })
        })
      }else {
    ...
    getDayData(){
      ...
      let data_row = []
      // 判断是否是当月
      let is_this_month = this.month===this.now.getMonth()+1&&this.now.getFullYear()<=this.year
      // 判断是否是以后
      let is_later = this.now.getFullYear()<this.year||(this.month>this.now.getMonth()+1&&this.now.getFullYear()===this.year)
      // 获取当前月份最大天数
      let current_day_num = this.getMaxDay()
      // 获取当前月份的1日是星期几
      let day_one = this.getDayOne();
      // 获取当上月份最大天数
      let prev_day_num = this.getMaxDay(this.month-1)
      // 增加上月的尾天
      for (let i=0;i<day_one;i++){
        let day_item ={num:prev_day_num-(i+1),class:"prev-month",isDisabled:false}
        if (is_later){
          day_item.class += " disabled"
          day_item.isDisabled=true
        }
        data_row.push(day_item)
      }
      data_row = data_row.sort(function(a,b){
        return a.num - b.num
      })
      // 加上每月的天数
      for (let i = 1;i <= current_day_num;i++){
        let day_item = {num:i,class:'',isDisabled:false}
        if (is_later){
          day_item.class = "disabled"
          day_item.isDisabled=true
        }else if(is_this_month){
          if(i<this.now.getDate()) {
            day_item.class = "available"
          }else{
            if (i===this.now.getDate()){
              day_item.class ="today disabled"
              day_item.num ="今天"
              day_item.isDisabled=true
            }else {
              day_item.class ="normal disabled"
              day_item.isDisabled=true
            }
          }
        }else {
      ...
      // 补上下月的天
      // 第一行
      for(let i = 1;data_row.length < 7;i++){
        let day_item = {num:i,class:"next-month",isDisabled:false}
        if (is_this_month||is_later){
          day_item.class += " disabled"
          day_item.isDisabled=true
        }
        data_row.push(day_item)
      ...
      // // 第二行
      if(data.length<6){
        let startDay = data_row[data_row.length-1].num+1
        data_row = []
        for(let i = startDay;data_row.length < 7;i++){
          let day_item = {num:i,class:"next-month",isDisabled:false}
          if (is_this_month||is_later){
            day_item.class += " disabled"
            day_item.isDisabled=true
    ...
    setYear(year){
      this.current_date_object.setFullYear(year)
      this.current_date=this.date_format(this.current_date_object)
      this.model="month"
    },
    setMonth(month){
      this.current_date_object.setFullYear(this.year,month-1)
      this.current_date=this.date_format(this.current_date_object)
      this.model="day"
    },
    setDay(day){
      this.current_date_object.setFullYear(this.year,this.month-1,day)
      this.current_date=this.date_format(this.current_date_object)
      this.isShow=false
    },
    setPrevMonthDay(day){
      ...
      if (this.month>0){
        ...
        this.current_date_object.setFullYear(this.year-1,11,day)
      }
      this.current_date=this.date_format(this.current_date_object)
      this.isShow=false
    },
  ...
  watch:{
    current_date(){
      let date_format = this.current_date.toString().split(",").join(" ").split("-").join(" ").split(" ")
      this.year=Number(date_format[0]&&date_format[0]>1500?date_format[0]:this.now.getFullYear())
      this.month=Number(date_format[1]&&date_format[1]>0&&date_format[1]<=12?date_format[1]:this.now.getMonth()+1);
      this.day=Number(date_format[2]&&date_format[2]>0&&date_format[2]<=this.getMaxDay(this.month,this.year)?date_format[2]:this.now.getDay());
      this.current_date_object=new Date(this.current_date)
      this.$emit("change",this.year+"-"+this.month+"-"+this.day)
      this.$emit("onChange")
    },
    year(){
      this.year_table_data=this.getYearData()
      this.month_table_data=this.getMonthData()
      this.day_table_data=this.getDayData()
    },
    month(){
      this.month_table_data=this.getMonthData()
      this.day_table_data=this.getDayData()
...
.el-table .hidden-columns, .el-table td.is-hidden > *,
